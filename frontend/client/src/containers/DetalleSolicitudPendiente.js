...
import { descargarArchivo,ramosSolicitudEncargado, volverSolicitudesPendientesDetalle } from "features/solicitud";
const DetalleSolicitudPendiente = () => {
    ...
    if(solicitud == null && detallesPendiente) return <Navigate to="/menu/"/>
    if(solicitud == null) return <Navigate to="/menu/"/>
    if( ramos != null) return <Navigate to="/menu/solicitudes-pendientes/detalle/ramos"/>
    const descargaHandler = e => {
        dispatch(descargarArchivo(solicitud.id));
    }
    const nextHandler = e => {
        dispatch(ramosSolicitudEncargado(solicitud.id));
    }
    const volverHandler = () => {
        dispatch(volverSolicitudesPendientesDetalle());
    }
...
export default DetalleSolicitudPendiente;